...
import { getFullName, normalizeSearchValue } from '@/features/super-admin/lib/userUtils';
...
export default function AddDivisionUsersModal({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await onAdded();
      onClose();
      toast.success(`Đã thêm ${selectedCount} thành viên vào mảng.`, {
        id: 'division-add-users-success',
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : '';
      toast.error(
        message ? `Không thể thêm thành viên: ${message}` : 'Không thể thêm thành viên.',
        { id: 'division-add-users-error' },
      );
    } finally {
      setIsAdding(false);
    }
