...
import { APP_ROUTES, getPublicBadmintonGamePath, withRouteQuery } from '@/constants/routes';
import {
  joinBadmintonGame,
  joinBadmintonGameAsGuest,
...
interface PublicBadmintonGameActionsProps {
  gameId: string;
  isExpired: boolean;
}

export default function PublicBadmintonGameActions({
  gameId,
  isExpired,
}: PublicBadmintonGameActionsProps) {
  ...
  async function handleAccountJoin() {
    try {
      ...
      const result = await joinBadmintonGame(gameId);
      toast.success('Đã tham gia kèo cầu lông.', { id: 'badminton-account-join-success' });
      router.push(result.managementPath);
      router.refresh();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Không thể tham gia kèo cầu lông.', {
        id: 'badminton-account-join-error',
      });
    } finally {
  ...
  async function handleGuestJoin(event: FormEvent<HTMLFormElement>) {
    ...
    if (!guestName.trim()) {
      toast.error('Vui lòng nhập tên khách.', { id: 'badminton-guest-name-error' });
      return;
    }

    try {
      setIsJoiningGuest(true);
      await joinBadmintonGameAsGuest(gameId, guestName, guestContact);
      setGuestName('');
      setGuestContact('');
      toast.success('Đã thêm khách tham gia.', { id: 'badminton-guest-join-success' });
      router.refresh();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Không thể thêm khách tham gia.', {
        id: 'badminton-guest-join-error',
      });
    } finally {
        ...
        {loading ? (
          ...
          <Link
            href={withRouteQuery(APP_ROUTES.login, {
              next: getPublicBadmintonGamePath(gameId),
            })}
