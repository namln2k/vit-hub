...
import {
  createBadmintonCostItem,
  deleteBadmintonCostItem,
  getBadmintonCostManagement,
  resetBadmintonPayment,
  updateBadmintonCostItem,
...
interface BadmintonCostManagementPanelProps {
  gameId: string;
...
function getPaymentForms(costs: BadmintonCostManagement): Record<string, PaymentForm> {
...
function getCostItemForms(costs: BadmintonCostManagement): Record<string, CostItemForm> {
  return Object.fromEntries(
...
export default function BadmintonCostManagementPanel({
  gameId,
  canManageCosts,
  costSharingEnabled,
}: BadmintonCostManagementPanelProps) {
  const [costs, setCosts] = useState<BadmintonCostManagement | null>(null);
  ...
  const loadCosts = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      setError('');
      const nextCosts = await getBadmintonCostManagement(gameId);
      setCosts(nextCosts);
  ...
  async function runCostAction(
    ...
    try {
      ...
      await action();
      toast.success(successMessage, { id: `badminton-cost-${actionId}-success` });
      await loadCosts();
    } catch (actionError) {
      toast.error(
        actionError instanceof Error ? actionError.message : 'Không thể cập nhật chia chi phí.',
        { id: `badminton-cost-${actionId}-error` },
      );
    } finally {
      setPendingAction('');
    }
  ...
  async function handleCreateCostItem(event: FormEvent<HTMLFormElement>) {
    ...
    if (!label || !Number.isFinite(amount) || amount < 0) {
      toast.error('Tên chi phí và số tiền phải hợp lệ.', {
        id: 'badminton-cost-create-validation',
      });
      return;
    }
    ...
    await runCostAction(
      'create-item',
      () => createBadmintonCostItem(gameId, { label, amount }),
      'Đã thêm chi phí.',
    );
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        onClick={() =>
                          void runCostAction(
                            `update-item-${item.id}`,
                            () =>
                              updateBadmintonCostItem(gameId, item.id, {
                                label: form.label,
                      ...
                      <button
                        ...
                        onClick={() =>
                          void runCostAction(
                            `delete-item-${item.id}`,
                            () => deleteBadmintonCostItem(gameId, item.id),
                            'Đã xóa chi phí.',
                          )
                        }
                        disabled={Boolean(pendingAction)}
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setPaymentForms({
                          ...
                          [payment.memberId]: {
                            ...form,
                            paymentStatus: event.target.value as BadmintonPaymentStatus,
                          },
                        })
                      }
                      disabled={Boolean(pendingAction)}
                      ...
                      <button
                        ...
                        onClick={() =>
                          void runCostAction(
                            ...
                            () =>
                              updateBadmintonPayment(gameId, payment.memberId, {
                      ...
                      <button
                        ...
                        onClick={() =>
                          void runCostAction(
                            ...
                            () => resetBadmintonPayment(gameId, payment.memberId),
                            'Đã reset thanh toán.',
                          )
                        }
                        disabled={Boolean(pendingAction)}
