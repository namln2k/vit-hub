...
import { getPublicBadmintonGame } from '@/features/badminton/server/badmintonRepository';
import PublicBadmintonGameActions from '@/features/badminton/components/PublicBadmintonGameActions';
import { notFound } from 'next/navigation';

interface PublicBadmintonGameRouteProps {
  params: Promise<{
    gameId: string;
  }>;
}
...
export default async function PublicBadmintonGameRoute({
  params,
}: PublicBadmintonGameRouteProps) {
  const { gameId } = await params;
  const game = await getPublicBadmintonGame(gameId);
    ...
    <main className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="mt-5">
          <PublicBadmintonGameActions gameId={game.id} isExpired={game.isExpired} />
        </section>
      </div>
    </main>
  );
