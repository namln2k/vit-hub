...
export async function POST(request: Request) {
  try {
    ...
    await addScopeMembers({ actorId: actor.id, scopeType, scopeId, userIds });
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!isManageableScopeType(scopeType) || !scopeId || userIds.length === 0) {
      return jsonResponse({ error: 'Dữ liệu xóa thành viên không hợp lệ.' }, 400);
    }

    if (!(await canManageScope(actor, { type: scopeType, id: scopeId }))) {
      return jsonResponse({ error: 'Bạn không có quyền xóa thành viên.' }, 403);
    }

    await endScopeMembers({ actorId: actor.id, scopeType, scopeId, userIds });
